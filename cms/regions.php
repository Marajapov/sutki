...
if ($isWhat == "edit") {
    ...
    if ($region_id != "") {
        $rw = $db->select_one(DB_PREFIX . "regions", "region_id=" . $region_id);
...
if ($isWhat == "delete") {
    ...
    $db->delete(DB_PREFIX . "regions", "region_id = " . $region_id);
...
// SET ORDER STATUS ************************************************************************************************************
if ($isWhat == "order") {
    $results = $db->select(DB_PREFIX . DB_PREFIX . "regions", "", "region_id", "ORDER BY region_id DESC");
    if ($results[0]["region_id"] > 0) {
        foreach ($results as $rw) {
            ...
            $db->update(DB_PREFIX . DB_PREFIX . "regions", $update, "region_id= " . $region_id);
        }
    }
}
...
if ($isWhat == "add") {
    ...
    $db->insert(DB_PREFIX . "regions", $insert);
...
<form action="<?php echo $form_action; ?>" method="post" enctype="multipart/form-data" name="addnew" id="addnew">
    <table cellpadding="0" cellspacing="0" border="0" style="height: 30px;">
        <tr>
            <td><input id="btnAddNew" value=" Добавить Нового Района " onclick="showDiv('dvForm');" type="button"
    ...
    <div id="dvForm" class="dvForm" style="display: <?php echo $edit ? 'block' : 'none'; ?>;">
        ...
        <table cellspacing="2" cellpadding="2" style="width: 100%;">
            ...
            <tr>
                <td style="text-align: right;">Район:</td>
                <td><input size="50" name="region_title" value="<?php echo $edit ? $region_title : ''; ?>"/></td>
            </tr>
    ...
    foreach ($results as $rw) {
        ...
        <tr>
            ...
            <td class="settingstools"><a
                href="?a=publish&region_id=<?php echo $rw['region_id']; ?>&status=<?php echo $rw['status'] == 1 ? "0" : "1"; ?>"><img
                src="images/tick_circle<?php echo $rw['status']; ?>.png" title="on/off"/></a></td>
            <td class="settingstools"><a href="?a=edit&region_id=<?php echo $rw['region_id']; ?>" alt="edit"><img
                src="images/page_white_edit.png" title="edit"/></a></td>
            <td class="settingstools"><a href="?a=delete&region_id=<?php echo $rw['region_id']; ?>"><img
                src="images/cross.png" title="delete"/></a></td>
