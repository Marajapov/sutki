...
if ($isWhat == "edit") {
    ...
    if ($type_id != "") {
        ...
        $row = $db->select(DB_PREFIX . "estate_types", "type_id=" . $type_id);
        $rw = $row[0];
...
if ($isWhat == "delete") {
    $type_id = $_GET['type_id'];
    $db->delete(DB_PREFIX . "estate_types", "type_id = " . $type_id);
}
...
if ($isWhat == "publish") {
    ...
    $db->update(DB_PREFIX . "estate_types", $update, "type_id = " . $type_id);
}
...
if ($isWhat == "add") {
    ...
    if ($error == "") {
        ...
        $db->insert(DB_PREFIX . "estate_types", $insert);
    }
}
...
?>
<h2 style="padding: 0px; margin: 0px;">Тип Недвижимости </h2>
<div class="clsFooter"
...
<table cellspacing="0" cellpadding="4">
    <tr id="list_title">
        ...
        <td align="center">Тип Недвижимости</td>
        <td align="center">Статус</td>
        <td align="center"></td>
        <td align="center"></td>
    </tr>
    <?php
    $results = $db->select(DB_PREFIX . "estate_types", "", "*", "ORDER BY type_title ASC");
    foreach ($results as $rw) {
        ...
        <td class="settingstools">
            <?php
            if (checkSubFiles(DB_PREFIX . "estates", " 	estate_type=" . $rw['type_id'], "estate_id") > 0) {
                echo "";
            } else {
                ?>
