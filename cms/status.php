...
if ($isWhat == "edit") {
    ...
    if ($type_id != "") {
        ...
        $row = $db->select(DB_PREFIX . "status", "type_id=" . $type_id);
        $rw = $row[0];
...
if ($isWhat == "delete") {
    $type_id = $_GET['type_id'];
    $db->delete(DB_PREFIX . "status", "type_id = " . $type_id);
}
...
if ($isWhat == "publish") {
    ...
    $db->update(DB_PREFIX . "status", $update, "type_id = " . $type_id);
}
...
if ($isWhat == "add") {
    ...
    if ($error == "") {
        ...
        $db->insert(DB_PREFIX . "status", $insert);
    }
}
...
?>
<h2 style="padding: 0px; margin: 0px;">Состояние</h2>
<div class="clsFooter"
...
<table cellspacing="0" cellpadding="4">
    ...
    <?php
    $results = $db->select(DB_PREFIX . "status", "", "*", "ORDER BY type_title ASC");
