...
if ($isWhat == "edit") {
    ...
    if ($type_id != "") {
        ...
        $row = $db->select(DB_PREFIX . "heating", "type_id=" . $type_id);
        $rw = $row[0];
...
if ($isWhat == "delete") {
    $type_id = $_GET['type_id'];
    $db->delete(DB_PREFIX . "heating", "type_id = " . $type_id);
}
...
if ($isWhat == "publish") {
    ...
    $db->update(DB_PREFIX . "heating", $update, "type_id = " . $type_id);
}
...
if ($isWhat == "add") {
    ...
    if ($error == "") {
        ...
        $db->insert(DB_PREFIX . "heating", $insert);
    }
}
...
?>
<h2 style="padding: 0px; margin: 0px;">Отопление </h2>
<div class="clsFooter"
...
<form action="<?php echo $form_action; ?>" method="post" name="frmForm" id="frmForm">
    <div id="dvNewButton" style="display: <?php echo $edit ? 'none' : 'block'; ?>;">
        <input id="btnAddNew" value=" Добавить Новый " onclick="showDiv('dvForm');" type="button"/>
    </div>
...
<table cellspacing="0" cellpadding="4">
    <tr id="list_title">
        ...
        <td align="center">Отопление</td>
        <td align="center">Статус</td>
        <td align="center"></td>
        <td align="center"></td>
    </tr>
    <?php
    $results = $db->select(DB_PREFIX . "heating", "", "*", "ORDER BY type_title ASC");
