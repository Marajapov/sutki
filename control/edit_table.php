...
function newTable (){	
	...
	table += "<table border='1' style='width:520px; margin-left:235px;'>";
	for (i = 1; i <= rows; i++) {
		table += "<tr>\n";
		for (t = 1; t <= cols; t++) {
...
function copytablevalue (){	
	document.getElementById("edit_table_hidden").value = document.getElementById("new_table").innerHTML;
}
        ...
        Столбец : | 		<input id="cols" value="2" maxLength="4" size="10" name="cols" class="textbox" />
        	...
        	<div id="new_table" style="display:<? if (strlen($tbl)>10) {?>block<? } else {?>none<? } ?>;" contenteditable="true">
			<?=$tbl;?>
        	</div>
            <input type="hidden" name="edit_table_hidden" id="edit_table_hidden" />
